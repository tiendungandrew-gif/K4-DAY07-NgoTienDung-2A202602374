        ...
        if sep in current_text:
            ...
            for s in splits:
                if len(s) > self.chunk_size:
                    if current_chunk:
                        docs.append(sep.join(current_chunk))
                        current_chunk = []
                        current_len = 0
                    docs.extend(self._split(s, next_seps))
                else:
                    addition_len = len(s) + (len(sep) if current_chunk else 0)
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        fixed = FixedSizeChunker(chunk_size=chunk_size, overlap=max(0, int(chunk_size * 0.1))).chunk(text)
